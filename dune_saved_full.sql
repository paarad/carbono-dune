...
, synthetic_histories_primary AS (
    SELECT
          p.week_end
        , SUM(sales) as sales
        , SUM(revenue) as revenue
    ...
    JOIN prices p
        ON block_time >= p.week_start AND block_time < p.week_end
    WHERE token_id > 39 AND token_id <= 52
    GROUP BY p.week_end
    ORDER BY p.week_end DESC
)

, synthetic_histories_secondary AS (
    SELECT
          p.week_end
        , SUM(sales) as sales
        , SUM(revenue) as revenue
...
-- 2025 Contract
, botto2025_primary AS (
    SELECT
          bp2025.block_time
        , bp2025.amount_original as sales
        , (bp2025.amount_original * 0.85) as revenue
        , cast(bp2025.number_of_items as integer) as quantity
        , token_id
    FROM nft.trades bp2025
    WHERE bp2025.block_time >= (SELECT start_date FROM dates)
    AND   bp2025.block_time < (SELECT end_date FROM dates)
    --Filtering to only Botto 2025 Contract:
    AND bp2025.nft_contract_address = 0x66cd3ede22a25eef3cc8799381b99b1d4f0983f8
    --Filtering to only primary sales:
    AND bp2025.seller IN  (0x000a837ddd815bcba0fa91a98a50aa7a3fa62c9c,
                        0x8c9f364bf7a56ed058fc63ef81c6cf09c833e656)
    ORDER BY bp2025.block_time DESC
)

, botto2025_secondary AS (
    SELECT
          bs2025.block_time
        , bs2025.amount_original as sales
        , (bs2025.amount_original / 10) as revenue
        , cast(bs2025.number_of_items as integer) as quantity
        , token_id
    FROM nft.trades bs2025
    WHERE bs2025.block_time >= (SELECT start_date FROM dates)
    AND   bs2025.block_time < (SELECT end_date FROM dates)
    --Filtering to only Botto 2025 Contract:
    AND bs2025.nft_contract_address = 0x66cd3ede22a25eef3cc8799381b99b1d4f0983f8
    --Filtering to only secondary sales:
    AND bs2025.seller NOT IN  (0x000a837ddd815bcba0fa91a98a50aa7a3fa62c9c,
                            0x8c9f364bf7a56ed058fc63ef81c6cf09c833e656)
    ORDER BY bs2025.block_time DESC
)

, cosmic_garden_primary AS (
    SELECT
          p.week_end
        , SUM(sales) as sales
        , SUM(revenue) as revenue
        , SUM(quantity) as quantity
    FROM botto2025_primary
    JOIN prices p
        ON block_time >= p.week_start AND block_time < p.week_end
    WHERE token_id >= 1 AND token_id <= 13
    GROUP BY p.week_end
...
, cosmic_garden_secondary AS (
    SELECT
          p.week_end
        , SUM(sales) as sales
        , SUM(revenue) as revenue
        , SUM(quantity) as quantity
    FROM botto2025_secondary
    JOIN prices p
        ON block_time >= p.week_start AND block_time < p.week_end
    WHERE token_id >= 1 AND token_id <= 13
    GROUP BY p.week_end
...
, liminal_thresholds_secondary AS (
    SELECT
          p.week_end
        , SUM(sales) as sales
        , SUM(revenue) as revenue
...
, attention_economy_secondary AS (
    SELECT
          p.week_end
        , SUM(sales) as sales
        , SUM(revenue) as revenue
...
-- 2026 Contract
, botto2026_primary AS (
    SELECT
          bp2026.block_time
        , bp2026.amount_original as sales
        , (bp2026.amount_original * 0.85) as revenue
        , cast(bp2026.number_of_items as integer) as quantity
        , token_id
    FROM nft.trades bp2026
    WHERE bp2026.block_time >= (SELECT start_date FROM dates)
    AND   bp2026.block_time < (SELECT end_date FROM dates)
    --Filtering to only Botto 2026 Contract:
    AND bp2026.nft_contract_address = 0x3cb787d48c34cca29653f54efc9112b4134b879d
    --Filtering to only primary sales:
    AND bp2026.seller IN  (0x000a837ddd815bcba0fa91a98a50aa7a3fa62c9c,
                        0x8c9f364bf7a56ed058fc63ef81c6cf09c833e656)
    ORDER BY bp2026.block_time DESC
)

, botto2026_secondary AS (
    SELECT
          bs2026.block_time
        , bs2026.amount_original as sales
        , (bs2026.amount_original / 10) as revenue
        , cast(bs2026.number_of_items as integer) as quantity
        , token_id
    FROM nft.trades bs2026
    WHERE bs2026.block_time >= (SELECT start_date FROM dates)
    AND   bs2026.block_time < (SELECT end_date FROM dates)
    --Filtering to only Botto 2026 Contract:
    AND bs2026.nft_contract_address = 0x3cb787d48c34cca29653f54efc9112b4134b879d
    --Filtering to only secondary sales:
    AND bs2026.seller NOT IN  (0x000a837ddd815bcba0fa91a98a50aa7a3fa62c9c,
                            0x8c9f364bf7a56ed058fc63ef81c6cf09c833e656)
    ORDER BY bs2026.block_time DESC
)

, collapse_aesthetics_primary AS (
    SELECT
          p.week_end
        , SUM(sales) as sales
        , SUM(revenue) as revenue
        , SUM(quantity) as quantity
    FROM botto2026_primary
    JOIN prices p
        ON block_time >= p.week_start AND block_time < p.week_end
    WHERE token_id >= 1 AND token_id <= 13
    GROUP BY p.week_end
...
, collapse_aesthetics_secondary AS (
    SELECT
          p.week_end
        , SUM(sales) as sales
        , SUM(revenue) as revenue
        , SUM(quantity) as quantity
    FROM botto2026_secondary
    JOIN prices p
        ON block_time >= p.week_start AND block_time < p.week_end
    WHERE token_id >= 1 AND token_id <= 13
    GROUP BY p.week_end
...
, weekly AS (
    SELECT
        ...
        , COALESCE(ROUND(b9p.sales,4),0) as synthetic_histories_primary_sales
        , COALESCE(ROUND(b9p.revenue,4),0) as synthetic_histories_primary_revenue
        , COALESCE(b9p.quantity,0) as synthetic_histories_primary_quantity
        , COALESCE(ROUND(b9s.sales,4),0) as synthetic_histories_secondary_sales
        , COALESCE(ROUND(b9s.revenue,4),0) as synthetic_histories_royalties
        , COALESCE(b9s.quantity,0) as synthetic_histories_secondary_quantity
        , COALESCE(ROUND(b10p.sales,4),0) as cosmic_garden_primary_sales
        , COALESCE(ROUND(b10p.revenue,4),0) as cosmic_garden_primary_revenue
        , COALESCE(b10p.quantity,0) as cosmic_garden_primary_quantity
        , COALESCE(ROUND(b10s.sales,4),0) as cosmic_garden_secondary_sales
        , COALESCE(ROUND(b10s.revenue,4),0) as cosmic_garden_royalties
        , COALESCE(b10s.quantity,0) as cosmic_garden_secondary_quantity
        , COALESCE(ROUND(b11p.sales,4),0) as liminal_thresholds_primary_sales
        , COALESCE(ROUND(b11p.revenue,4),0) as liminal_thresholds_primary_revenue
        , COALESCE(b11p.quantity,0) as liminal_thresholds_primary_quantity
        , COALESCE(ROUND(b11s.sales,4),0) as liminal_thresholds_secondary_sales
        , COALESCE(ROUND(b11s.revenue,4),0) as liminal_thresholds_royalties
        , COALESCE(b11s.quantity,0) as liminal_thresholds_secondary_quantity
        , COALESCE(ROUND(b12p.sales,4),0) as semantic_drift_primary_sales
        , COALESCE(ROUND(b12p.revenue,4),0) as semantic_drift_primary_revenue
        , COALESCE(b12p.quantity,0) as semantic_drift_primary_quantity
        , COALESCE(ROUND(b12s.sales,4),0) as semantic_drift_secondary_sales
        , COALESCE(ROUND(b12s.revenue,4),0) as semantic_drift_royalties
        , COALESCE(b12s.quantity,0) as semantic_drift_secondary_quantity
        , COALESCE(ROUND(b13p.sales,4),0) as attention_economy_primary_sales
        , COALESCE(ROUND(b13p.revenue,4),0) as attention_economy_primary_revenue
        , COALESCE(b13p.quantity,0) as attention_economy_primary_quantity
        , COALESCE(ROUND(b13s.sales,4),0) as attention_economy_secondary_sales
        , COALESCE(ROUND(b13s.revenue,4),0) as attention_economy_royalties
        , COALESCE(b13s.quantity,0) as attention_economy_secondary_quantity
        , COALESCE(ROUND(b14p.sales,4),0) as collapse_aesthetics_primary_sales
        , COALESCE(ROUND(b14p.revenue,4),0) as collapse_aesthetics_primary_revenue
        , COALESCE(b14p.quantity,0) as collapse_aesthetics_primary_quantity
        , COALESCE(ROUND(b14s.sales,4),0) as collapse_aesthetics_secondary_sales
        , COALESCE(ROUND(b14s.revenue,4),0) as collapse_aesthetics_royalties
        , COALESCE(b14s.quantity,0) as collapse_aesthetics_secondary_quantity
    ...
    LEFT JOIN synthetic_histories_primary b9p
        ON b9p.week_end = p.week_end
    LEFT JOIN synthetic_histories_secondary b9s
        ON b9s.week_end = p.week_end
    LEFT JOIN cosmic_garden_primary b10p
        ON b10p.week_end = p.week_end
    LEFT JOIN cosmic_garden_secondary b10s
        ON b10s.week_end = p.week_end
    LEFT JOIN liminal_thresholds_primary b11p
        ON b11p.week_end = p.week_end
    LEFT JOIN liminal_thresholds_secondary b11s
        ON b11s.week_end = p.week_end
    LEFT JOIN semantic_drift_primary b12p
        ON b12p.week_end = p.week_end
    LEFT JOIN semantic_drift_secondary b12s
        ON b12s.week_end = p.week_end
    LEFT JOIN attention_economy_primary b13p
        ON b13p.week_end = p.week_end
    LEFT JOIN attention_economy_secondary b13s
        ON b13s.week_end = p.week_end
    LEFT JOIN collapse_aesthetics_primary b14p
        ON b14p.week_end = p.week_end
    LEFT JOIN collapse_aesthetics_secondary b14s
        ON b14s.week_end = p.week_end
    ...
    GROUP BY p.week, p.week_number, p.week_period, p.week_end, p.week_start, p.botto_price, p.eth_to_botto, bb.botto_burnt,
             ...
             b9p.sales, b9s.sales, b9p.revenue, b9s.revenue, b9p.quantity, b9s.quantity,
             b10p.sales, b10s.sales, b10p.revenue, b10s.revenue, b10p.quantity, b10s.quantity,
             b11p.sales, b11s.sales, b11p.revenue, b11s.revenue, b11p.quantity, b11s.quantity,
             b12p.sales, b12s.sales, b12p.revenue, b12s.revenue, b12p.quantity, b12s.quantity,
             b13p.sales, b13s.sales, b13p.revenue, b13s.revenue, b13p.quantity, b13s.quantity,
             b14p.sales, b14s.sales, b14p.revenue, b14s.revenue, b14p.quantity, b14s.quantity,
